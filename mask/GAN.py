...
class GANModelDesc(ModelDescBase):
    def collect_variables(self, g_scope='gen', m_scope='mappingnet', d_scope='discrim'):
        """
        ...
        assert self.d_vars
        self.m_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, m_scope)
        assert self.m_vars
...
class GANTrainer(TowerTrainer):
    ...
    def _build_gan_trainer(self, input, model):
        ...
        with tf.name_scope('optimize'):
            ...
            with tf.control_dependencies([g_min]):
                m_min = optm.minimize(model.g_loss, var_list=model.m_vars, name='m_op')
                with tf.control_dependencies([m_min]):
    ...
    def _build_multigpu_gan_trainer(self, input, model, num_gpu):
        ...
        with tf.name_scope('optimize'):
            ...
            with tf.control_dependencies([g_min]):
                m_min = optm.minimize(model.g_loss, var_list=model.m_vars, colocate_gradients_with_ops=True, name='m_op')
            with tf.control_dependencies([m_min]):
...
class DTrainer(TowerTrainer):
    ...
    def __init__(self, input, model, num_gpu=1):
        ...
        if num_gpu <= 1:
            ...
            print('create multi-gpu setting')
    ...
    def _build_gan_trainer(self, input, model):
        ...
        self.train_op = opt.minimize(model.d_loss, var_list=model.d_vars, name='d_op')

class GTrainer(TowerTrainer):
