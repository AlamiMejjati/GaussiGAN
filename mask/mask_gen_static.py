...
class Model(GANModelDesc):
    ...
    def build_graph(self, img, box, mask, bbx, z, z2):
        ...
        with tf.name_scope('preprocess'):
            ...
            self.lr = tf.get_variable('learning_rate', initializer=LR, trainable=False)
            self.lrm = tf.identity(ratio * self.lr, 'learning_rate_mn')
        ...
        with argscope([Conv2D, Conv2DTranspose, tf.layers.conv2d]):
            ...
            with tf.variable_scope('gen'):
                ...
                with tf.variable_scope('genlandmarks'):
                    thets_cyc = self.transform_mu_sigma(zgt_cyc, mucann, sigmacann)
                    mus_cyc = mucann
                    sigmas_cyc = sigmacann
            ...
            vizN('A_recon', [mask, mask, gen_mask, gen_mask_rot], [colorize_landmark_maps(pose_embeddings[-1]),
        ...
        add_moving_summary(G_loss_mask, D_loss_mask,  FM_loss_mask, mask_recon_loss, mask_recon_loss_bis,
                           mask_recon_loss_rot_bis, self.lr, self.lrm, gm_cycle_loss)
        add_tensor_summary(tf.identity(thets[0,0]), ['scalar'])

    def optimizer(self):

        return tf.train.AdamOptimizer(self.lr, beta1=0.5, epsilon=1e-3), tf.train.AdamOptimizer(self.lrm, beta1=0.5, epsilon=1e-3)
...
if __name__ == '__main__':
    ...
    logdir = os.path.join('train_log', args.dataset, basename,
                          'Ep%d_r%.1f_L%d_Lm%d_Lgm%d_Lgmrot%d_Lcyc%d_ange%d_nbl%d' %(nb_epochs, ratio, LAMBDA, LAMBDA_m,
                                                                                     LAMBDA_gm, LAMBDA_gm_rot,
