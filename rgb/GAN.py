...
class GANModelDesc(ModelDescBase):
    def collect_variables(self, g_scope='gen', d_scope='discrim'):
        ...
        assert self.g_vars
        for y in self.g_vars:
            if not 'genim/sencim' in y.name:
                self.g_vars_mines_e.append(y)
...
class GANTrainer(TowerTrainer):
    ...
    def _build_gan_trainer(self, input, model):
        ...
        with tf.name_scope('optimize'):
            ...
            with tf.control_dependencies([g_min]):
                g_min_e = opt.minimize(model.z_loss, var_list=model.g_vars_mines_e, name='g_mines_e_op')
            with tf.control_dependencies([g_min_e]):
    ...
    def _build_multigpu_gan_trainer(self, input, model, num_gpu):
        ...
        with tf.name_scope('optimize'):
            ...
            with tf.control_dependencies([g_min]):
                g_min_e = opt.minimize(model.z_loss, var_list=model.g_vars_mines_e,
                                       colocate_gradients_with_ops=True, name='g_mines_e_op')
            with tf.control_dependencies([g_min_e]):
